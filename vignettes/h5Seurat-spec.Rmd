...
<details>
  <summary>Rationale</summary>
  Unlike most languages, <a href=`r plink("logical")`>logicals</a> (or booleans) can take one of _three_ values: `TRUE`, `FALSE`, or [`NA`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/NA.html); as such, an extra integer value is needed to handle the additional logical value. Typically, these values are stored as <a href=`r hlink("ENUM")`>enums</a> with mappings between the logical representation and integer value. However, [some implementations of HDF5 do not support enums](http://docs.h5py.org/en/stable/special.html#enumerated-types) and thus lose the logical representation. Since the mappings are lost, all logicals are stored as integers instead.
</details>
